...
await (async () => {
  console.warn('now', (new Date).toLocaleDateString());

  // Injections
  let appCtx = AppContext.getInstance()
  const services = ['chromeHelper, logger', 'hitoApi', 'useCredentialStore']
  const { chromeHelper, logger, hitoApi: api, useCredentialStore } = appCtx.makeMany(services)

  // Credentials store
  let store = useCredentialStore()
  await store.init()
