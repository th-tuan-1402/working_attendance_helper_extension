...
/**
 * Background task that runs periodically to check in or check out
 * if needed.
 */
export async function loop() {
    console.log("[hitoExtension] Background task is running...")
    const now = new Date()

    // Credentials store
    let credentials = (await localStorage.getArray("credentials")) ?? [];
    console.log(`[hitoExtension] Total number of credentials: ${credentials.length}`)
    credentials.forEach(async (credential) => {
        console.log(`[hitoExtension] Checking credential: ${credential.userId}...`)

        // Check if the access token is still valid
        console.log("[hitoExtension] Checking access token...")
        if (!(await checkKintaiToken(credential))) {
            console.log("[hitoExtension] Access token is not valid, refreshing...")
            await onRefreshToken(credential);
        }
        console.log("[hitoExtension] Access token is valid")

        await onSyncStatus(credential);

        // Check if it's time to check in
        console.log("[hitoExtension] Checking if it's time to check in...")
        if (shouldCheckIn(now, credential)) {
            console.log("[hitoExtension] It's time to check in")
            await onCheckIn(credential)
        }
        // Check if it's time to check out
        else if (shouldCheckOut(now, credential)) {
            console.log("[hitoExtension] It's time to check out")
            await onCheckOut(credential)
        }
        else {
            console.log("[hitoExtension] It's not time to check in or check out")
        }
    })
}
